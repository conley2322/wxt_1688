...
// --- 标签池 ---
const tags = [
  { text: '优质供应商', color: '#ffffff', bgcolor: '#52c41a' },
  { text: '需议价', color: '#ffffff', bgcolor: '#faad14' },
  { text: '已下单', color: '#ffffff', bgcolor: '#1890ff' },
  { text: '样品待确认', color: '#ffffff', bgcolor: '#722ed1' },
  { text: '质量有问题', color: '#ffffff', bgcolor: '#f5222d' },
]
const insertTag = db.prepare('INSERT OR IGNORE INTO tags (id, text, color, bgcolor, visible, created_user_id) VALUES (?, ?, ?, ?, 1, ?)')
const tagIds = []
for (const t of tags) {
  const id = generateId()
  insertTag.run(id, t.text, t.color, t.bgcolor, String(userIds[0]))
  tagIds.push(id)
}
...
const insertConfig = db.prepare('INSERT OR IGNORE INTO system_configs (id, config_key, config_value, description, updated_by) VALUES (?, ?, ?, ?, ?)')
...
const insertAnnouncement = db.prepare('INSERT OR IGNORE INTO announcements (id, title, content, status, published_by) VALUES (?, ?, ?, ?, ?)')
...
const insertUserConfig = db.prepare('INSERT OR IGNORE INTO user_configs (id, user_id, config_key, config_value) VALUES (?, ?, ?, ?)')
...
const insertSC = db.prepare('INSERT OR IGNORE INTO supplier_comments (id, supplier_id, user_id, text) VALUES (?, ?, ?, ?)')
const scIds = []
...
const insertST = db.prepare('INSERT OR IGNORE INTO supplier_tags (id, supplier_id, tag_id, tag_user) VALUES (?, ?, ?, ?)')
...
const insertCoop = db.prepare('INSERT OR IGNORE INTO supplier_cooperations (id, supplier_id, user_id) VALUES (?, ?, ?)')
...
const insertPCI = db.prepare('INSERT OR IGNORE INTO product_comment_images (id, comment_id, file_name, file_path, file_size, uploaded_by) VALUES (?, ?, ?, ?, ?, ?)')
...
const insertSCI = db.prepare('INSERT OR IGNORE INTO supplier_comment_images (id, comment_id, file_name, file_path, file_size, uploaded_by) VALUES (?, ?, ?, ?, ?, ?)')
...
const insertLog = db.prepare('INSERT OR IGNORE INTO operation_logs (id, user_id, action, target_type, target_id, detail, ip_address) VALUES (?, ?, ?, ?, ?, ?, ?)')
for (const l of logs) {
